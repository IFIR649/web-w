...
if ($_SERVER['REQUEST_METHOD'] === 'GET') {
    ...
    if (isset($_GET['id'])) {
        ...
        // Consultar los maestros para el dropdown
        $queryMaestros = "SELECT id_maestro, CONCAT(nombre, ' ', apellido_paterno, ' ', apellido_materno) AS nombre_completo FROM maestros";
        $resultMaestros = $conn->query($queryMaestros);
        $maestrosList = [];
        while ($row = $resultMaestros->fetch_assoc()) {
            $maestrosList[] = $row;
        }

        // Consultar los alumnos para el dropdown
    ...
    try {
        ...
        if (!empty($_POST['alumnos'])) {
            ...
            $stmt_alumno = $conn->prepare("INSERT INTO grupo_participantes (id_grupo, id_maestro, matricula) VALUES (?, ?, ?)");

            foreach ($_POST['alumnos'] as $matricula) {
                $stmt_alumno->bind_param('iii', $id_grupo, $id_maestro, $matricula);
                $stmt_alumno->execute();
            }
        ...
        $conn->commit();
        echo "<script>alert('Grupo, horarios y alumnos actualizados exitosamente.'); window.location.href = '../../../groups.php';</script>";
    } catch (Exception $e) {
        // Revertir la transacción en caso de error
        $conn->rollback();
        echo "<script>alert('Error al actualizar el grupo: " . $e->getMessage() . "');</script>";
    }
                ...
                <div id="alumnosContainer">
                    <?php foreach ($alumnos as $alumno): ?>
                        <div class="d-flex mb-2 alumno-item">
                            ...
                            <form method="POST" action="delete-student.php" onsubmit="return confirm('¿Estás seguro de eliminar este alumno del grupo?');">
...
<body>
    ...
    <script>
        ...
        function addAlumno() {
            ...
            div.innerHTML = `
                <select name="alumnos[]" class="form-select me-2" required>
